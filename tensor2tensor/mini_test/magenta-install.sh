...
conda create -n magenta2 python=2.7
...
set +e
source activate magenta2
set -e
if [[ $(conda info --envs | grep "*" | awk '{print $1}') != "magenta2" ]]; then
  err 'Did not successfully activate the magenta2 conda environment'
fi
